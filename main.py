...
@app.get('/person/{p_id}')
def get_person(p_id: int, status_code=200):
...
def change_person(person: Person):
    ...
    if len(person_list) > 0:
        ...
        return HTTPException(status_code=404, detail=f'Person with id {person.id} doesnt exist')
...
def delete_person(p_id: int):
    ...
    if len(person) > 0:
        ...
        raise HTTPException(status_code=404, detail=f"there is no person with that id {p_id}")
